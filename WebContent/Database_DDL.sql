...
COMMENT ON COLUMN MIS_CAT.TITLE IS '제목';
...
, CONSTRAINT AA_ADT_ADAPTION_FK FOREIGN KEY (ADT_ADAPTION) REFERENCES RESPONSE_TYPE (YESNO)
...
COMMENT ON COLUMN ADT_APPLY.ADT_ADAPTION IS '입양 경험';
...
-- ACT_IMG Table Create SQL
CREATE TABLE ACT_IMG
( ACT_IMG_CODE    VARCHAR2(100)
, ACT_CODE        VARCHAR2(100)
, IMG_FILE        VARCHAR2(100)
, CONSTRAINT AI_CODE_PK PRIMARY KEY (ACT_IMG_CODE)
, CONSTRAINT AI_ACT_CODE_FK FOREIGN KEY (ACT_CODE) REFERENCES ACT_REG (ACT_CODE)
);
COMMENT ON TABLE ACT_IMG IS '활동별 이미지 등록';
COMMENT ON COLUMN ACT_IMG.ACT_IMG_CODE IS '시퀀스';
COMMENT ON COLUMN ACT_IMG.ACT_CODE IS '활동 식별키';
COMMENT ON COLUMN ACT_IMG.IMG_FILE IS '이미지 파일명';
